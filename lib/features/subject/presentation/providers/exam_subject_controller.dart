...
class ExamSubjectController extends _$ExamSubjectController {
  ...
  @override
  AsyncValue<Paged<ExamEntity>> build(int idSubject,ExamType type) {
    _link ??= ref.keepAlive();
    ref.onCancel(() {
  ...
  Future<List<ExamEntity>> _fetch(int page) async {
    final uc = ref.read(getListSubjectExamUsecaseProvider);
    final either = await uc.getListExam(idSubject,type, page: page);
